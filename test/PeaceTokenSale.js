...
var PeaceTokenSale = artifacts.require("./PeaceTokenSale.sol");

contract("PeaceTokenSale", function (accounts) {
  var tokenInstance;
  ...
  it("initializes the contract with the correct values", function () {
    return PeaceTokenSale.deployed()
    ...
    return PeaceToken.deployed()
      ...
      .then(async function (balance) {
        assert.equal(
          balance.toNumber(),
          999990,
          "returns all unsold peace tokens to admin"
        );
        // Check that the contract has no balance

        await web3.eth
          .getBalance(tokenSaleInstance.address)
          .then((bal) => {
            console.log("New balance Value, ", bal);
            balance = bal;
          })
          .catch(console.error);
